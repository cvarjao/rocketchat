...
#bitnami_repo_url="$(helm repo list -o json | jq -r '.[] | select (.name == "bitnami") | .url')"

if [ "${stable_repo_url}" != "https://kubernetes-charts.storage.googleapis.com" ]; then
  helm repo add stable https://kubernetes-charts.storage.googleapis.com
fi
#if [ "${bitnami_repo_url}" != "https://charts.bitnami.com" ]; then
...
helm template rocketchat-prod ".charts/${CHART_MONGODB_FILE}" \
  ...
  --set 'replicaSet.replicas.secondary=1' \
...
jq -M '(.objects[] | select(.kind == "StatefulSet")).spec.template.spec.containers[].image = "mongodb:4.2.3"' '.openshift/mongodb.json' | sponge '.openshift/mongodb.json'
...
jq '.parameters = [{"name": "INSTANCE", "required": true},{"name": "MONGODB_REPLICA_KEY", "required": true, "generate": "expression", "from": "[a-zA-Z]{1}[a-zA-Z0-9]{8}[a-zA-Z]{1}"},{"name": "MONGODB_ROOT_PASSWORD", "required": true, "generate": "expression", "from": "[a-zA-Z]{1}[a-zA-Z0-9]{12}[a-zA-Z]{1}"},{"name": "MONGODB_PASSWORD", "required": true, "generate": "expression", "from": "[a-zA-Z]{1}[a-zA-Z0-9]{12}[a-zA-Z]{1}"}]' '.openshift/mongodb.json' | sponge '.openshift/mongodb.json'
...
jq '.parameters = [{"name": "INSTANCE", "required": true}]' '.openshift/rocketchat.json' | sponge '.openshift/rocketchat.json'
